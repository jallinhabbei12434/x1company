document.addEventListener("DOMContentLoaded", () => {
  ...
  function handlePhoneVerification(e) {
    ...
    sendCodeBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> VINCULANDO...'

    // Vincular número via Z-API
    linkPhoneNumber(phoneConfirm)
  }
  ...
  async function linkPhoneNumber(phone) {
    try {
      ...
      // Configuração da Z-API (credenciais corretas)
      const zapiConfig = {
      ...
      console.log("Vinculando número via Z-API:", phone)
      console.log("Código gerado:", generatedCode)

      // Primeiro, vincular o número
      const linkResponse = await linkNumberToInstance(zapiConfig)

      if (linkResponse.success) {
        toastr.success("Número vinculado! Enviando código...")

        // Aguardar um pouco e então enviar o código
        setTimeout(async () => {
          const smsResponse = await sendVerificationCode(zapiConfig)

          if (smsResponse.success) {
            toastr.success("Código enviado via WhatsApp!")
            showSMSCodeSection()
          } else {
            console.error("Erro ao enviar SMS:", smsResponse.error)
            toastr.warning("Número vinculado, mas erro ao enviar código.")
            toastr.info(`Código para teste: ${generatedCode}`)
            showSMSCodeSection()
          }
        }, 2000)
      } else {
        console.error("Erro ao vincular número:", linkResponse.error)
        toastr.error("Erro ao vincular número. Tente novamente.")
  ...
  async function linkNumberToInstance(config) {
    try {
      // Endpoint para vincular número (conectar via telefone)
      const response = await fetch(`https://api.z-api.io/instances/${config.instanceId}/token/${config.token}/phone`, {
        method: "POST",
        headers: {
      ...
      console.log("Resposta vinculação Z-API:", data)
      ...
      console.error("Erro na vinculação Z-API:", error)
      return { success: false, error: error.message }
    }
  }

  async function sendVerificationCode(config) {
    try {
      const message = `🎰 Papi Bet - Seu código de verificação é: ${generatedCode}\n\nNão compartilhe este código com ninguém.\n\nVálido por 10 minutos.`
      ...
      const data = await response.json()
      console.log("Resposta envio SMS Z-API:", data)
      ...
      console.error("Erro no envio SMS Z-API:", error)
      return { success: false, error: error.message }
    }
  }
